...
export default function ClientLayout({ title, children }) {
  const navItems = ['Overview', 'Estimates', 'Customers', 'Settings'];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "system-ui, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      <aside style={{
        ...
        <nav style={{ padding: '12px 0', flex: 1 }}>
          {navItems.map((item) => (
            <NavItem key={item} label={item} route={navRoutes[item]} />
          ))}
        </nav>
      </aside>
      ...
      <div style={{
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '14px', color: '#64748b' }}>Acme AB</span>
            <div style={{
              width: '34px',
              height: '34px',
              borderRadius: '50%',
